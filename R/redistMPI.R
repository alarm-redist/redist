...
ecutsMPI <- function(procID = procID, params = params, adj = adj, total_pop = total_pop, init_plan = init_plan, swaps = swaps) {
    ...
    if (params$verbose) {
        ...
        cat("gredist.mcmc.mpi(): Automated Redistricting Simulation Using
    ...
    ## Run gredist preprocessing function
    ...
    if (nloop > 1) {
        if (procID == tempadj[1]) {
            gredist.combine.mpi(savename = savename, nloop = nloop,
    ...
    if (params$verbose) {
        ...
        cat("gredist.mcmc.mpi() simulations finished.\n", append = TRUE)
        sink()
    }
    ## End function
}

#' Combine successive runs of \code{gredist.mcmc.mpi}
#'
#' \code{gredist.combine.mpi} is used to combine successive runs of
#' \code{gredist.mcmc.mpi} into a single data object
#'
#' @usage gredist.combine.mpi(savename, nloop, nthin, tempadj)
#'
...
#' \code{gredist.mcmc.mpi}.
#'
#' @details This function allows users to combine multiple successive runs of
#' \code{gredist.mcmc.mpi} into a single \code{gredist} object for analysis.
#'
#' @return \code{gredist.combine.mpi} returns an object of class "gredist".
#' The object \code{gredist} is a list that contains the following components (the
...
#'
#' ## Run the algorithm
#' gredist.mcmc.mpi(adj = fl25_adj, total_pop = fl25$pop,
#'     init_plan = init_plan, nsims = 10000, nloops = 2, savename = "test")
#' out <- gredist.combine.mpi(savename = "test", nloop = 2,
#'     nthin = 10, tempadj = tempAdjMat)
#' }
#' @concept post
#' @export
gredist.combine.mpi <- function(savename, nloop, nthin, tempadj) {
...
#'
#' \code{gredist.mcmc.mpi} is used to simulate Congressional redistricting
...
#' Chain.
#'
#' @return \code{gredist.mcmc.mpi} returns an object of class "gredist". The object
...
#' }
#' @export
gredist.mcmc.mpi <- function(adj, total_pop, nsims, ndists = NA,
                            init_plan = NULL,
