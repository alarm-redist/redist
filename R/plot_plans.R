...
#' group_by(plans, draw) |> #'     summarize(pop_dev = max(abs(total_pop/mean(total_pop) - 1))) |> #'     redist.plot.hist(pop_dev)
#'
#' @concept plot
#' @export
...
#' plans <- redist_smc(iowa, nsims = 100, silent = TRUE)
#' plans |> #'     mutate(comp = distr_compactness(iowa)) |> #'     group_by(draw) |> #'     summarize(pop_dev = max(abs(total_pop/mean(total_pop) - 1)),
#'         comp = comp[1]) |> #'     redist.plot.scatter(pop_dev, comp)
#'
#' @concept plot
#' @export
...
redist.plot.distr_qtys <- function(
    ...
    if (isFALSE(sort) || sort == "none") {
        plans <- dplyr::group_by(plans, .data$draw) |>             dplyr::mutate(.distr_no = as.factor(.data$district))
    } else {
        ord <- if (sort == "asc") {
            1
        } else if (sort == "desc") {
        ...
        plans <- dplyr::group_by(plans, .data$draw) |>             dplyr::mutate(.distr_no = as.factor(rank(ord * {{ qty }}, ties.method = "first")))
    }
...
redist.plot.trace <- function(plans, qty, district = 1L, ...) {
    ...
    plans <- as.data.frame(plans) |>         filter(!is.na(.data$chain)) |>         mutate(chain = as.factor(.data$chain)) |>         arrange(.data$draw)
