...
#' 
pick_a_plan <- function(plans, map, counties = NULL, comp = NULL, maximize_comp = TRUE,  cut_point = 0.3){
  ...
  while(prior_rows > 0.05 * orig_rows ){
    ...
    plans <- plans %>% filter(parity <= quantile(parity, cut_point))
    
    if(maximize_comp){
      plans <- plans %>% filter(compactness >= quantile(comp, 1 - cut_point))
    } else {
      plans <- plans %>% filter(compactness <= quantile(comp, cut_point))
    }
