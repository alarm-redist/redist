...
#'
#' \code{redist_mcmc.anneal} simulates congressional redistricting plans
...
#'
#' @export
redist_mcmc.anneal <- function(adjobj, popvec, ndists = NULL,
                               initcds = NULL,
    ...
    if(verbose){
        ...
        cat("redist_mcmc.anneal(): Automated Redistricting Simulation Using
...
#'
#' Combine files generated by redist_mcmc.anneal()
#'
...
#' Combine successive runs of \code{redist_mcmc}
#'
#' \code{redist.combine} is used to combine successive runs of \code{redist_mcmc}
...
#' \code{redist_mcmc} into a single \code{redist} object for analysis.
#'
...
#'
#' ## Run the algorithm
#' alg_253 <- redist_mcmc(adjobj = algdat.pfull$adjlist,
...
#'
#' \code{redist_mcmc} is used to simulate Congressional redistricting
...
#' Chain.
#'
#' @return \code{redist_mcmc} returns an object of class "redist". The object
#' \code{redist} is a list that contains the following components (the
#' inclusion of some components is dependent on whether tempering
#' techniques are used):
#' \item{partitions}{Matrix of congressional district assignments generated by the
...
#' }
#' @export
redist_mcmc <- function(adjobj, popvec, nsims, ndists = NULL, initcds = NULL,
    ...
    if(verbose){
        ...
        cat("\n", append = TRUE)
        cat(divider, append = TRUE)
        cat("redist_mcmc(): Automated Redistricting Simulation Using
         Markov Chain Monte Carlo\n\n", append = TRUE)
    }
...
#' alg_253_20_st <- redist_mcmc(adjobj = algdat.p20$adjlist,
