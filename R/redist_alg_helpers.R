...
validate_initial_region_sizes_mat <- function(init_dvals_mat, ndists, nsims, init_num_regions){
    ...
    if(!is.matrix(init_dvals_mat)){
        cli::cli_abort("{.arg init_dvals_mat} must be a matrix.")
    }
    if (nrow(init_dvals_mat) != ndists)
        cli::cli_abort("{.arg init_dvals_mat} must have {.arg ndists} rows.")
    if (ncol(init_dvals_mat) != nsims)
        cli::cli_abort("{.arg init_dvals_mat} must have {.arg nsims} columns.")

    # check each value is an integer
    if(any(init_dvals_mat != round(init_dvals_mat))){
        cli::cli_abort("Each entry of {.arg init_dvals_mat} must sum an integer!")
    }
    ...
    if(any(colSums(init_dvals_mat[1:init_num_regions, ,drop=FALSE]) != ndists))
        cli::cli_abort("Each column of {.arg init_dvals_mat} must sum to {.arg ndists}.")
    # check that init_num_regions+1 to the end of the matrix is 0
    if(any(init_dvals_mat[(init_num_regions+1):ndists,] != 0))
        cli::cli_abort("Rows {.arg init_num_regions}+1 to {.arg ndists} of each column of {.arg init_dvals_mat} must be 0.")
    # check that 1:init_num_regions is between 1 and ndists
    if(any(init_dvals_mat[1:init_num_regions,] < 1))
        cli::cli_abort("All values in rows 1 to {.arg init_num_regions} of each column of {.arg init_dvals_mat} must be greater than 0.")
    if(any(init_dvals_mat[1:init_num_regions,] > ndists))
        cli::cli_abort("All values in rows 1 to {.arg init_num_regions} of each column of {.arg init_dvals_mat} must be less or equal to {.arg ndists}.")
...
OLD_validate_cut_sizes <- function(ndists, num_splits,
    ...
    if(any(min_region_cut_sizes < 0)){
        cli::cli_abort("Minimum region cut sizes must be 1 or greater!")
    }
    if(any(min_region_cut_sizes > rev(seq_len(ndists-1)))){
        cli::cli_abort("Minimum region cut sizes must be less than {.arg ndists}!")
    }
    if(any(max_region_cut_sizes < 0)){
        cli::cli_abort("Maximum region cut sizes must be 1 or greater!")
    }
    if(any(max_region_cut_sizes > rev(seq_len(ndists-1)))){
        cli::cli_abort("Maximum region cut sizes must be less than {.arg ndists}!")
    }
    ...
    for (i in 1:num_splits) {
        ...
        if(all(potentential_sizes <= min_region_cut_sizes[i])){
            cli::cli_abort("For split {i} all region sizes are less than {min_region_cut_sizes[i]}")
        }
