...
p = make_iter_plot("rhat", expression(hat(R))) +
    ...
    theme(legend.position = "bottom", plot.margin = unit(rep(1, 4), "mm"))
...
make_valid_plot = function(M, conf = 0.9, by_chain = TRUE) {
    ...
    d_hist = map_dfr(edge_rg, function(k) {
        redist_ci(pl_smc, edges == k, 1, conf = conf, by_chain = by_chain) %>%
            ...
            mutate(edges = k, true = weighted.mean(by_plan(pl_enum$edges, n) == k, w))
    })

    ggplot(d_hist, aes(edges)) +
        geom_col(aes(y = true), fill = PAL[1]) +
        geom_pointrange(
            aes(y = est, ymin = low, ymax = high),
            color = "#444444",
            linewidth = 0.7,
            fatten = 1.5
        ) +
...
p = make_valid_plot(100) +
    ...
    plot_layout(nrow = 1) &
    theme(plot.margin = margin(l = 0.8, unit = "mm"))
