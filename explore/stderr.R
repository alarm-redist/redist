...
plans <- redist_smc(ia, 250, runs = 20, seq_alpha = 0.9, silent = T) %>%
    mutate(
        dem = group_frac(ia, dem_08, dem_08 + rep_08),
...
calc_se <- function(x1) {
    ...
    tibble(true = std_err_chains, est1 = std_errs_anc, est2 = std_errs_neff)
}
...
res <- map_dfr(cli::cli_progress_along(1:100), function(i) {
    ...
    N <- length(x1)
    d <- tibble(
        ancestor = factor(attr(plans, "diagnostics")[[1]]$ancestors, levels = 1:N),
        resid = x1 - mean(x1)
    )
    cross_sum <- expand_grid(
        rename_with(d, \(x) paste0(x, "_i")),
    ...
    tibble(mean = mean(x1), se1 = sqrt(var_est_ancestor), se2 = sqrt(var_est_neff))
})
