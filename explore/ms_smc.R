...
pl_ms = redist_mergesplit(
    ...
    subset_sampled() |>
    summarize(polsby = mean(polsby), e_dem = sum(dem > 0.5), .by = any_of(c("draw", "chain")))
summary(pl_ms)
...
pl_smc = redist_smc(
    ...
    subset_sampled() |>
    summarize(polsby = mean(polsby), e_dem = sum(dem > 0.5), .by = c("draw", "chain"))
summary(pl_smc)
...
t.test(pl_smc$polsby > 0.20 & pl_smc$polsby < 0.21, pl_ms$polsby > 0.20 & pl_ms$polsby < 0.21)
