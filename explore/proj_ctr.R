...
map |>
    ...
    st_as_sf() |>
    summarize(
        across(X:pa_y, weighted.mean, w = vap),
    ...
    geom_sf(
        aes(fill = vap_black / vap, alpha = 1609^2 * vap / area_land),
        linewidth = 0,
        color = NA
    ) +
    geom_district(aes(group = cd_2020), fill = NA, is_coverage = TRUE) +
    ...
    geom_segment(
        aes(x = X, y = Y, xend = X + pc_x / 5, yend = Y + pc_y / 5),
...
edges <- dplyr::as_tibble(map) %>%
    ...
    dplyr::mutate(
        i = sapply(.data$origins, function(x) x[1]),
...
ggplot(map) +
    ...
    geom_sf(
        aes(linewidth = cut, alpha = cut),
        data = filter(edge_cuts, cut > 0.05),
        color = "black"
    ) +
    geom_district(
        aes(group = cd_2010),
...
ggplot(map) +
    ...
    geom_district(aes(group = cd_2020), fill = NA, is_coverage = TRUE) +
    geom_segment(
        aes(x = X, y = Y, xend = X - dX / 7, yend = Y - dY / 7),
