...
std::pair<bool, EdgeCut> USTSampler::attempt_to_find_valid_tree_split(
    ...
    // Try to draw a tree
    bool tree_drawn = attempt_to_draw_tree_on_region(rng_state, plan, region_to_split);
    // return false if unsuccessful
    if (!tree_drawn)
        return std::make_pair(false, EdgeCut());
...
std::pair<bool, EdgeCut> USTSampler::attempt_to_find_valid_tree_mergesplit(
    ...
    // Try to draw a tree
    bool tree_drawn =
        attempt_to_draw_tree_on_merged_region(rng_state, plan, merge_region1, merge_region2);
    // return false if unsuccessful
    if (!tree_drawn)
        return std::make_pair(false, EdgeCut());
