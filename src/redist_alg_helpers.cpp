...
PlanEnsemble get_plan_ensemble(
    ...
    if (num_regions == 1) {
        return PlanEnsemble(map_params, arma::sum(map_params.pop), nsims, sampling_space, pool,
                            verbosity);
    } else {
...
std::unique_ptr<PlanEnsemble> get_plan_ensemble_ptr(
    ...
    std::vector<RNGState> &rng_states, RcppThread::ThreadPool &pool, int const verbosity) {
    if (num_regions == 1) {
        return std::make_unique<PlanEnsemble>(map_params, arma::sum(map_params.pop), nsims,
...
double get_log_number_linking_edges(Rcpp::List const &adj_list, arma::uvec const &counties,
