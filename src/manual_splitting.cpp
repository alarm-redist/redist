...
List draw_a_tree_on_a_region(
    ...
    bool split_district_only = region_dvals.max() == 1;
    ...
    for (int i = 0; i < V; i++){
        ignore[i] = plan.region_ids(i) != region_id_to_draw_tree_on;
    }
...
List perform_merge_split_steps(
    ...
    // Create a plan object
    Plan plan = Plan(region_ids.col(0), region_dvals.col(0), V, ndists, total_pop);
    Plan new_plan = Plan(dummy_region_ids.col(0), dummy_region_dvals.col(0), V, ndists, total_pop);
