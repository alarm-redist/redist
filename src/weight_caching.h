...
class WeightCache {
  ...
  public:
    WeightCache(int const ndists, IntPlanAttribute &this_plan_order_added,
                DoublePlanAttribute &region_cache_values)
        : this_plan_order_added(this_plan_order_added), region_cache_values(region_cache_values)
    // pair_map(ndists*3 - 6)
    {};

    IntPlanAttribute this_plan_order_added; // This stores the region order counter when region
                                            // value was cached
    DoublePlanAttribute
        region_cache_values; // This stores compactness + constraint score for each region
    ...
    attempt_to_get_region_value(RegionID const region_id,
...
class WeightCacheEnsemble {
  ...
  public:
    WeightCacheEnsemble(bool const using_caching, MapParams const &map_params, int const nsims,
                        double const rho, SamplingSpace const sampling_space);
...
double compute_or_fetch_log_region_compactness(Plan const &plan, RegionID const region_id,
                                               MapParams const &map_params, double const rho,
                                               bool const using_caching,
