...
double Plan::compute_log_region_spanning_trees(MapParams const &map_params,
    ...
    log_st += compute_log_county_level_spanning_tree_eigen(map_params.g, map_params.counties,
...
double PlanMultigraph::compute_log_multigraph_tau(int const num_regions,
    ...
    if (counties_on) {
        double new_tau =
            compute_hierarchical_log_multigraph_tau_eigen(num_regions, scoring_function);
        ...
        if (num_component_regions <= 1) {
            ...
            while (curr_index < all_pairs.size() &&
                ...
                if (pair_val.admin_adjacent) {
                    ...
                    throw Rcpp::exception(
                        "Error in compute_hierarchical_log_multigraph_tau_eigen");
                }
...
double PlanMultigraph::compute_hierarchical_log_multigraph_tau(
    ...
    for (int component_id = 0; component_id < num_county_connected_components; component_id++) {
        ...
        if (num_component_regions <= 1) {
            ...
            // Now make the graph laplacian matrix
            arma::mat laplacian_minor(num_component_regions - 1, num_component_regions - 1,
                                      arma::fill::zeros);
    ...
    // Now we compute spanning trees across components
    arma::mat component_laplacian_minor(num_county_connected_components - 1,
            ...
            while (
                ...
                if (pair_val.admin_adjacent) {
                    ...
                    REprintf(
                        "Error! in BASE PLAN TYPE NON ADMIN ADJACENT HIER MERGE INVALID!\n");
                    throw Rcpp::exception(
                        "Error in compute_hierarchical_merged_log_multigraph_tau_eigen");
                }
...
double PlanMultigraph::compute_hierarchical_merged_log_multigraph_tau(
    ...
    // Now we compute spanning trees across components
    arma::mat component_laplacian_minor(merged_num_admin_connected_components - 1,
                                        merged_num_admin_connected_components - 1,
                                        arma::fill::zeros);
...
PlanMultigraph::PlanMultigraph(MapParams const &map_params,
    ...
    : map_params(map_params), counties_on(map_params.num_counties > 1),
      ...
      WAIT_laplacian_minor(0, 0), WAIT_merged_laplacian_minor(0, 0) {
...
void PlanMultigraph::prep_for_calculations(int const num_regions) {
    ...
    if (!need_to_compute_multigraph_taus || num_regions <= 2) {
        ...
        WAIT_merged_laplacian_minor =
            arma::mat(num_regions - 2, num_regions - 2, arma::fill::none);
        return;
    }
}
...
std::vector<EdgeCut> TreeSplitter::get_all_valid_pop_edge_cuts_in_directed_tree(
    const MapParams &map_params, Tree const &ust, const int root, TreePopStack &stack,
...
std::pair<bool, EdgeCut> TreeSplitter::attempt_to_find_edge_to_cut(
    ...
    Plan const &plan, int const split_region1, int const split_region2, Tree const &ust,
...
TreeSplitter::select_edge_to_cut(ScoringFunction const &scoring_function, Tree const &ust,
...
std::pair<bool, EdgeCut>
NaiveTopKSplitter::select_edge_to_cut(ScoringFunction const &scoring_function, Tree const &ust,
                                      RNGState &rng_state, std::vector<EdgeCut> &valid_edges,
                                      bool save_selection_prob) const {
...
std::pair<bool, EdgeCut> UniformValidSplitter::select_edge_to_cut(
    ScoringFunction const &scoring_function, Tree const &ust, RNGState &rng_state,
