...
WeightCacheEnsemble::WeightCacheEnsemble(bool const using_caching, MapParams const &map_params,
    ...
    : using_caching(using_caching), ndists(map_params.ndists), rho(rho), nsims(nsims),
    ...
    for (size_t i = 0; i < nsims; i++) {
        DoublePlanAttribute weight_cache_values(flattened_all_cached_weight_values, ndists * i,
                                                ndists * (i + 1));
        IntPlanAttribute cache_region_order_added(flattened_all_region_order_added, ndists * i,
                                                  ndists * (i + 1));

        weight_cache_ptr_vec[i] = std::make_unique<WeightCache>(
            ndists, cache_region_order_added, weight_cache_values);
    }
};
...
double compute_or_fetch_log_region_compactness(Plan const &plan, RegionID const region_id,
    ...
    double compactness_term =
        (rho - 1) * plan.compute_log_region_spanning_trees(map_params, region_id);
