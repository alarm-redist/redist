...
//' @param pop Vector of mapping vertices to population (0-indexed)
//' @param plan A plan object
//' @param region_to_split The label of the region in the plan object we're attempting to split
//' @param lower Acceptable lower bounds on district population
//' @param upper Acceptable upper bounds on district population
//' @param target Target population (probably Total population of map/Num districts )
...
bool cut_regions(Tree &ust, int k_param, int root,
    ...
    bool pop_testing = new_region1_pop/new_region1_d >= lower && new_region1_pop/new_region1_d <= upper &&
        new_region2_pop/new_region2_d >= lower && new_region2_pop/new_region2_d <= upper;

    if(!pop_testing){
        Rprintf("AHHHHH BADDDDDD!!!!\n\n");
    }
...
//' Attempts to split a multi-district into two new regions with valid population
//' bounds
//'
//' Attempts to split a multi-district into two new regions where both regions
//' have valid population bounds. Does this by drawing a spanning tree uniformly
//' at random then calling `cut_regions` on that. If the split it successful it
//' returns true and modifies `plan` and `new_region_labels` accordingly. This
//' is based on the `split_map` function in smc.cpp
//'
//'
//' @title Attempt Generalized Region split of multi-district
//'
//' @param g A graph (adjacency list) passed by reference
//' @param ust A directed tree object (this will be cleared in the function so
//' whatever it was before doesn't matter)
//' @param counties Vector of county labels
...
void generalized_split_maps(
        ...
        const std::vector<double> &unnormalized_weights,
    ...
    uvec iters(M, fill::zeros); // how many actual iterations
    uvec original_ancestor_uniques(M); // unique original ancestors
    uvec parent_index_uniques(M); // unique parent indicies
    ...
    std::discrete_distribution<> index_sampler(
            unnormalized_weights.begin(), unnormalized_weights.end()
        );

    std::vector<double> p = index_sampler.probabilities();

    // for(int i=0;i < M, i++){
    //     normalized_weights_to_fill_in.at(i) = ;
    // }

    // record the normalized weights the sampler is using
    int nw_index = 0;
    bool print_weights = M < 12 && verbosity > 1;
    if(print_weights){
    Rprintf("Unnormalized weights are: ");
    for (auto w : unnormalized_weights){
        Rprintf("%.4f, ", w);
    }
    ...
    pool.parallelFor(0, M, [&] (int i) {
        ...
        while (!ok) {
            ...
            // else update the new plan
            new_plans_vec[i] = proposed_new_plan;

        }

        // update the log incremental weights
        log_incremental_weights[i] = compute_log_incremental_weight(g, new_plans_vec[i]);
    ...
    if(print_weights){
        ...
        for (auto w : log_incremental_weights){
            Rprintf("%.4f, ", w);
        }
        Rprintf("\n");
    }
...
List generalized_smc_plans(
    ...
    // Now create data necessary to split the maps
    ...
    if (verbosity >= 1) {
        ...
        Rcout << "maps with " << N << " districts and population between "
              << lower << " and " << upper << " using " << ncores << " cores.\n";
        if (cg.size() > 1){
    ...
    std::vector<double> unnormalized_weights(M, 1.0);
    ...
    for(int n=0; n<N-1; n++){
        ...
        for(int j=0; j<M; j++){
            ...
            unnormalized_weights[j] = std::exp(
                log_incremental_weights_mat[n][j] // + std::log(unnormalized_weights[j])
            );
