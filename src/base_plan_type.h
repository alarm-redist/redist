...
class Plan {
  ...
  public:
    ...
    std::pair<bool, int> draw_tree_on_region(const MapParams &map_params,
                                             const int region_to_draw_tree_on, Tree &ust,
                                             std::vector<bool> &visited,
                                             std::vector<bool> &ignore, int &root,
...
class RegionPairHash {
  ...
  public:
    ...
    RegionPairHash(int const ndists)
        ...
        if (ndists < 2)
            throw Rcpp::exception("Region Pair hash must have at least 2 elements!\n");
    }
...
class TreeSplitter {
  ...
  public:
    ...
    std::vector<EdgeCut> get_all_valid_pop_edge_cuts_in_directed_tree(
        MapParams const &map_params, Tree const &ust, const int root, TreePopStack &stack,
