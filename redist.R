...
#'
#' @usage redist.mcmc(algorithm, adjobj, popvec, nsims, ndists, initcds,
...
#' verbose, num_hot_steps, num_annealing_steps, num_cold_steps, ncores)
#'
#' @param algoirthm The algorithm to run. Default is \code{mcmc}, the other
#' implemented is \code{anneal}.
...
#' Default is 40000. This is only used for algoritm anneal.
...
#' Default is 20000. This is only used for algoritm anneal.
#' @param ncores The number of cores available to parallelize over. Default is 1. 
#' This is only used for algoritm anneal.
#'
...
redist.mcmc <- function(algorithm = 'mcmc', adjobj, popvec, nsims, ndists = NULL, initcds = NULL,
  ...
  preprocout <- redist.preproc(adjobj = adjobj, popvec = popvec,
                               ...
                               temper = ifelse(algorithm=='mcmc',temper,FALSE),
                               constraint = constraint, constraintweights = constraintweights,
                               betaseq = ifelse(betaseq, 'powerlaw'),
                               betaseqlength = ifelse(algorithm=='mcmc', betaseqlength, 10),
                               betaweights = ifelse(algorithm=='mcmc',betaweights,NULL),
                               adjswaps = ifelse(algorithm=='mcmc',adjswaps,TRUE),
