test_that('redist_bud runs on Iowa data', {
  result <- redist_bud(ia, nsims = 500)

  expect_s3_class(result, 'redist_plans')
  expect_equal(ncol(get_plans_matrix(result)), 51)
  expect_equal(nrow(get_plans_matrix(result)), 99)
})

test_that('redist_bud respects population bounds', {
  set.seed(1)
  result <- redist_bud(ia, nsims = 200)

  pop_bounds <- attr(ia, 'pop_bounds')
  pops <- result |>
    dplyr::group_by(draw) |>
    dplyr::summarize(
      min_pop = min(total_pop),
      max_pop = max(total_pop)
    )

  expect_true(all(pops$min_pop >= pop_bounds[1]))
  expect_true(all(pops$max_pop <= pop_bounds[3]))
})

test_that('redist_bud with verbose output works', {
  expect_output(
    redist_bud(ia, nsims = 10, verbose = TRUE),
    'Balanced Up-Down Walk'
  )
})

test_that('redist_bud with silent output works', {
  expect_silent(redist_bud(ia, nsims = 10, silent = TRUE))
})

test_that('BUD can produce different plans', {
  set.seed(1)
  result <- redist_bud(ia, nsims = 1000, thin = 1, verbose = FALSE)

  plans_mat <- get_plans_matrix(result)

  # Count unique plans
  n_unique <- ncol(unique(plans_mat, MARGIN = 2))

  expect_true(n_unique >= 10)
  expect_true(ncol(plans_mat) == 1001)
})

test_that('all generated plans are contiguous', {
  result <- redist_bud(ia, nsims = 500, verbose = FALSE)
  plans_mat <- get_plans_matrix(result)
  cont <- apply(plans_mat, 2, \(x) max(contiguity(adj = ia$adj, x)))
  expect_equal(max(cont), 1)
})

test_that('redist_bud runs with constraints', {
  skip_on_cran()

  constr <- redist_constr(ia) |>
    add_constr_pop_dev(strength = 10)

  result <- redist_bud(ia, nsims = 500, constraints = constr, verbose = FALSE)

  expect_s3_class(result, 'redist_plans')
  expect_equal(ncol(get_plans_matrix(result)), 51)

  constr <- redist_constr(ia) |>
    add_constr_splits(strength = 5, admin = region)

  result <- redist_bud(ia, nsims = 500, constraints = constr, verbose = FALSE)

  expect_s3_class(result, 'redist_plans')
  expect_equal(ncol(get_plans_matrix(result)), 51)
})

test_that('all plans have valid district structure', {
  result <- redist_bud(ia, nsims = 1000, verbose = FALSE)
  plans_mat <- get_plans_matrix(result)

  ndists <- attr(ia, 'ndists')

  expect_true(all(apply(plans_mat, 2, function(x) length(unique(x))) == ndists))
  expect_true(all(apply(plans_mat, 2, function(x) x >= 1 & x <= ndists)))
})

test_that('longer chain runs without crashing', {
  skip_on_cran()

  result <- redist_bud(ia, nsims = 500, verbose = FALSE)

  expect_s3_class(result, 'redist_plans')
  expect_equal(ncol(get_plans_matrix(result)), 51)
})

test_that('thinning works correctly', {
  # With thin=5 and nsims=1000, should get 1000/5 + 1 = 201 plans
  result <- redist_bud(ia, nsims = 1000, thin = 5, verbose = FALSE)

  expect_equal(ncol(get_plans_matrix(result)), 201)
})

test_that('warmup burns initial samples', {
  result <- redist_bud(ia, nsims = 500, warmup = 10, verbose = FALSE)

  expect_s3_class(result, 'redist_plans')
  # 1 reference + 49 sampled (with thin=10 default)
  expect_equal(ncol(get_plans_matrix(result)), 50)
})

test_that('init_plan parameter works', {
  result <- redist_bud(ia, nsims = 200, init_plan = ia$cd_2010, verbose = FALSE)

  plans_mat <- get_plans_matrix(result)

  expect_true(all(vctrs::vec_group_id(ia$cd_2010) == vctrs::vec_group_id(plans_mat[, 1])))
})

test_that('BUD produces valid plans on 4x4 grid', {
  skip_on_cran()

  set.seed(42)
  result <- redist_bud(grid, nsims = 500, init_plan = grid$init, verbose = FALSE)

  cut_edge_counts <- comp_edges_rem(result, shp = grid) |> by_plan(ndists = 4)

  expect_true(all(cut_edge_counts %in% c(8, 10, 11, 12)))
  expect_true(length(unique(cut_edge_counts)) >= 2)
})
