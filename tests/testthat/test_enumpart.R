...
if (
    !(file.exists(system.file("enumpart/enumpart", package = "redist")) ||
...
test_that("enumpart can read in partial data", {
    ...
    samp <- redist.read.enumpart(out_path = file.path(dir, "sample"), skip = 5)
    ...
    samp <- redist.read.enumpart(out_path = file.path(dir, "sample"), n_max = 8)
