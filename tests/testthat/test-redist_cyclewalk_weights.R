...
test_that("redist_cyclewalk works with single edge weight", {
    ...
    set.seed(02139)
    result <- redist_cyclewalk(fl_map, nsims = 200, edge_weights = ew)
...
test_that("edge weights can be specified in either direction", {
    ...
    set.seed(02139)
    result1 <- redist_cyclewalk(fl_map, nsims = 100, edge_weights = ew1)

    set.seed(02139)
    result2 <- redist_cyclewalk(fl_map, nsims = 100, edge_weights = ew2)
...
test_that("edge weights prints custom weight count when verbose", {
    ...
    expect_output(
        redist_cyclewalk(fl_map, nsims = 50, edge_weights = ew, verbose = TRUE, silent = FALSE),
...
test_that("edge_weights must be a list", {
    expect_error(
        redist_cyclewalk(fl_map, nsims = 50, edge_weights = "not a list"),
        "must be a list"
    )
})
...
test_that("edge_weights entries must be lists", {
    ...
    expect_error(
        redist_cyclewalk(fl_map, nsims = 50, edge_weights = ew),
...
test_that("adjacency check works correctly with 0-indexed adj list", {
    ...
    expect_silent(
        redist_cyclewalk(fl_map, nsims = 50, edge_weights = ew1, silent = TRUE)
    )

    expect_silent(
        redist_cyclewalk(fl_map, nsims = 50, edge_weights = ew2, silent = TRUE)
    )
})
...
test_that("edge weights work with different compactness values", {
    ...
    set.seed(02139)
    result1 <- redist_cyclewalk(fl_map, nsims = 200, edge_weights = ew, compactness = 0.5)

    set.seed(02139)
    result2 <- redist_cyclewalk(fl_map, nsims = 200, edge_weights = ew, compactness = 2.0)
...
test_that("edge weights work with constraints", {
    ...
    result <- redist_cyclewalk(fl_map, nsims = 200, edge_weights = ew, constraints = constr)
...
test_that("all plans remain contiguous with edge weights", {
    ...
    result <- redist_cyclewalk(fl_map, nsims = 500, edge_weights = ew)
...
test_that("empty edge_weights list works", {
    ...
    result <- redist_cyclewalk(fl_map, nsims = 100, edge_weights = ew)
