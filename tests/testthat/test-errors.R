...
test_that("redist_mergesplit rejects invalid arguments", {
    # nsims = 0 trips the warmup check first
    expect_error(redist_mergesplit(iowa_map, nsims = 0, warmup = 0, silent = TRUE), "greater than")
    expect_error(redist_mergesplit(iowa_map, nsims = 10, compactness = -1, silent = TRUE), "non-negative")
    expect_error(redist_mergesplit(iowa_map, nsims = 10, adapt_k_thresh = 1.5, silent = TRUE), "0, 1")
    expect_error(
        redist_mergesplit(iowa_map, nsims = 5, warmup = 10, silent = TRUE),
        "greater than"
    )
    expect_error(
...
test_that("redist_mergesplit rejects malformed init_plan", {
    expect_error(
        redist_mergesplit(iowa_map, nsims = 5, warmup = 0,
                          init_plan = 1:3, silent = TRUE),
        "as long as"
    )
    wrong_ndists <- rep(1L, n_iowa)
    expect_error(
        redist_mergesplit(iowa_map, nsims = 5, warmup = 0,
                          init_plan = wrong_ndists, silent = TRUE),
...
test_that("redist_flip rejects invalid arguments", {
    expect_error(
        redist_flip(iowa_map, nsims = 5, constraints = list(), verbose = FALSE),
        "redist_constr"
    )
    expect_error(
        redist_flip(iowa_map, nsims = 5, thin = -1, verbose = FALSE),
        "nonnegative"
    )
    expect_error(
        redist_flip(iowa_map, nsims = 5, init_plan = rep(1L, n_iowa),
                    verbose = FALSE),
        "same number of districts"
    )
})
...
test_that("redist_shortburst rejects invalid arguments", {
    expect_error(
        redist_shortburst(iowa_map, scorer_frac_kept(iowa_map),
                          compactness = -1, max_bursts = 1, verbose = FALSE),
        "non-negative"
    )
    expect_error(
        redist_shortburst(iowa_map, scorer_frac_kept(iowa_map),
                          adapt_k_thresh = -0.1, max_bursts = 1, verbose = FALSE),
        "0, 1"
    )
    expect_error(
        redist_shortburst(iowa_map, scorer_frac_kept(iowa_map),
                          max_bursts = 0, verbose = FALSE),
        "positive"
    )
    expect_error(
        redist_shortburst(iowa_map, scorer_frac_kept(iowa_map),
                          constraints = list(), max_bursts = 1, verbose = FALSE),
        "redist_constr"
    )
})
...
test_that("validate_redist_map flags non-redist_map and missing adjacency", {
    expect_error(redist:::validate_redist_map(list()), "redist_map")
    expect_error(redist:::validate_redist_map(iris), "redist_map")

    # a redist_map-classed object that's missing the adjacency column attribute
    no_adj <- iowa_map
    attr(no_adj, "adj_col") <- NULL
    expect_error(redist:::validate_redist_map(no_adj), "adjacency")
})
