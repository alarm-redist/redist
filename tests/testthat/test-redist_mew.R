...
test_that("MEW warmup and thinning work correctly", {
    ...
    # Test with thinning
    out2 <- redist_mew(fl_map, 100, warmup = 20, thin = 4,
...
test_that("MEW init_plan validation works", {
    # Invalid length
    expect_error(redist_mew(fl_map, 10, warmup = 0, init_plan = c(1, 2, 3)),
                "as long as the number of units")

    # Wrong number of districts
    bad_plan <- rep(1:2, length.out = 25)
    expect_error(redist_mew(fl_map, 10, warmup = 0, init_plan = bad_plan),
