...
test_that("chains with explicit ncores", {
    ...
    expect_true("chain" %in% names(result))
    expect_length(unique(result$chain), 2)
    expect_length(unique(result$draw[result$chain == 1]), 30)
    expect_length(unique(result$draw[result$chain == 2]), 30)
})
...
test_that("init_plan as single vector replicates across chains", {
    ...
    expect_length(unique(result$chain[!is.na(result$chain)]), 2)
...
test_that("edge weights work with chains", {
    ...
    expect_false(anyNA(result$mcmc_accept))
})
...
test_that("summary works for cyclewalk diagnostics", {
    ...
    expect_type(diagn, "list")
    expect_length(diagn, 2) # 2 chains
})
