# Tests for redist_bud edge weights
# fl_map already created in setup.R

test_that('redist_bud works with NULL edge_weights', {
  set.seed(02139)
  result <- redist_bud(fl_map, nsims = 200)
...
test_that('redist_bud works with single edge weight', {
  ew <- list(list(edge = c(1, 2), weight = 2.0))

  set.seed(02139)
  result <- redist_bud(fl_map,
...
test_that('redist_bud works with multiple edge weights', {
  ew <- list(
...
test_that('edge weights can be specified in either direction', {
  ...
  set.seed(02139)
  result1 <- redist_bud(fl_map,
    nsims = 100, edge_weights = ew1
  )

  set.seed(02139)
  result2 <- redist_bud(fl_map,
...
test_that('counties parameter auto-generates edge weights', {
  ...
  set.seed(123)
  result <- redist_bud(ia, 200, counties = iowa$region, silent = TRUE)
...
test_that('edge_weights as number sets county weight multiplier', {
  ...
  set.seed(123)
  result <- redist_bud(ia, 200, counties = iowa$region, edge_weights = 5, silent = TRUE)
  expect_s3_class(result, 'redist_plans')

  set.seed(123)
  result2 <- redist_bud(ia, 20, counties = iowa$region, edge_weights = 20, silent = TRUE)
  expect_s3_class(result2, 'redist_plans')
})

test_that('edge_weights as number requires counties', {
  expect_error(
    redist_bud(fl_map, nsims = 50, edge_weights = 5),
    'requires.*counties'
  )
})
