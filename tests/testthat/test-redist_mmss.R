test_that("redist_mms works", {
    ...
    nsims <- 10
    out <- redist_mmss(fl_map, nsims, nsims %/% 2, init_plan = plans_10[, 1], silent = TRUE)
    par <- redist.parity(as.matrix(out), total_pop = pop)

    expect_equal(range(as.matrix(out)), c(1, 3))
    expect_true(all(par <= 0.1))

    out <- redist_mmss(fl_map, 20, 5, thin = 4, init_plan = plans_10[, 1], silent = TRUE)
...
test_that("Additional constraints work", {
    ...
    plans <- redist_mmss(iowa_map, 100, 20, init_plan = iowa$cd_2010,
