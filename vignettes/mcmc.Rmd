...
fl_map$plan <- plan
fl_map <- redist_map(fl25, existing_plan = plan, pop_tol = 0.1, total_pop = pop)
constr <- redist_constr(fl_map) %>% 
    add_constr_edges_rem(0.02)
set.seed(1)
sims <- redist.flip(fl_map, nsims = 6, constraints = constr)
```
...
```{r}
sims <- redist_flip(map_ia, nsims = 100, pop_tol = 0.05)
```
...
sims_comp <- redist_flip(map_ia, sims = 100, constraints = constr)
```
...
flip_chains <- lapply(1:nchains, function(x){
  redist_flip(map_ia, nsims = nsims, init_plan = 'rsg', ndists = 4,
