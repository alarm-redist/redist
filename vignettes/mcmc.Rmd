---
title: "Redistricting with MCMC and RSG"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Redistricting with MCMC and RSG}
...
simulations. The package comes loaded with data for simple testing and with
functions to simulate redistricting and to run diagnostics on the redistricting
plans created.  These data form the basis for small-scale validations of
sampling methods in 
[Automated Redistricting Simulation Using Markov Chain Monte Carlo](https://imai.fas.harvard.edu/research/files/redist.pdf). 
For larger scale validation, see 
...
```{r include=F}
alg_mcmc <- redist.mcmc(adj = adjlist, total_pop = fl25$pop,
                        ndists = 3, nsims = 10000)
```
```{r eval=F}
alg_mcmc <- redist.mcmc(adj = adjlist, total_pop = fl25$pop,
                        ndists = 3, nsims = 10000, savename = "redist.mcmc")
```
...
```{r}
data(fl25_enum)
init_plan <- fl25_enum$plans[, 7173]
```
...
For simple runs of the algorithm, the most important pieces of the output are `plans` and `distance_parity`.  The `plans` object is a matrix with `ndist` rows and `nsims` columns. Each column is one redistricting plan, where the numbers go from 0 to n-1, with each number represents the district assignment.
...
```{r, eval=F}
library(Rmpi)
redist.mcmc.mpi(adjobj = adjlist, popvec = fl25$pop,
                nsims = 10000,  ndists = 3, savename = "redist.mcmc.mpi")
```
...
Herem we opt to intialize using the `RSG` algorithm, though the default is to use the `SMC` algorithm.
```{r}
mcmc_chains <- lapply(1:nchains, function(x){
          redist.mcmc(adjobj = adjlist,  popvec = fl25$pop, 
                      nsims = nsims, init_plan = 'rsg', ndists = 3)
})
```
...
The adjacency list, via `adj`, is created as in [Adjacency-Based Redistricting](#adj), `total_pop` is the population of the districts, `ndists` is the number of districts, and `pop_tol` is the allowed population parity.  A `pop_tol` of `0.05` allows for 5\% population parity. In easier redistricting questions, a small `maxiter` is typically sufficient, but in large maps, values may be closer to `50,000` to ensure that a solution is found.
```{r}
rsg <- redist.rsg(adj = adjlist, total_pop = fl25$pop,
                  ndists = 3, pop_tol = 0.05,  maxiter = 5000)
```
...
```{r}
seg <- redist.segcalc(algout = alg_mcmc, group_pop = fl25$BlackPop,
                      total_pop = fl25$pop)
```
