...
```{r include=F}
alg_mcmc <- redist.mcmc(adj = algdat.pfull$adjlist,
...
```{r}
init_plan <- algdat.pfull$cdmat[,1]
```
...
```{r}
rsg <- redist.rsg(adj = algdat.pfull$adjlist,
                  total_pop = algdat.pfull$precinct.data$pop,
                  ndists = 3,
                  pop_tol = 0.05, 
                  maxiter = 5000)
...
seg <- redist.segcalc(algout = alg_mcmc, 
                      group_pop = algdat.pfull$precinct.data$blackpop,
                      total_pop = algdat.pfull$precinct.data$pop)
```
