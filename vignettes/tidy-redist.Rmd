...
let `redist_map()` know about this by providing the `existing_plan` argument, and
we use `max(cd)` to count the total number of districts.
```{r}
east_tx_map = redist_map(east_tx, pop_tol=0.05, existing_plan=cd, total_pop=pop)
print(east_tx_map)
```
...
```{r}
houston_area = filter(east_tx_map, cd %in% c(2, 7, 9, 18, 22, 29)) 
print(houston_area)
```
...
plot(houston_area, fill=hisp/pop)
```
...
plot(houston_counties, fill=hisp/pop)
```
...
```{r message=F}
houston_plans = redist_smc(houston_cores, 100, counties=county, verbose=F) %>%
    add_reference(houston_cores$cd)
print(houston_plans)
```

Since district numbers in simulated plans are by and large random, we often want
to renumber the simulated districts to match the district numbers in the original plan,
something which is easily accomplished by the function `match_numbers()`:
```{r}
houston_plans = match_numbers(houston_plans, houston_cores$cd)
print(houston_plans)
```
This function also creates a new column, which records, for each plan, the
fraction of the total population which lives in the same district under the
reference plan.
...
print(houston_plans)
```
If we wanted, we could sort and number districts according to one of these
statistics using the `number_by()` function.

From there, it's easy to summarize summary statistics by plan.
```{r}
...
hist(min_distr, n_coalition)
redist.plot.distr_qtys(houston_plans, pct_hisp)
# don't sort; use existing district numbers
redist.plot.distr_qtys(houston_plans, pct_hisp, sort=F)
```
...
redist.plot.plan(houston_orig_plans, draw=5, geom=houston_area)
```
...
```{r message=F}
houston = east_tx %>%
    redist_map(pop_tol=0.05, existing_plan=cd) %>%
...
houston_plans = redist_smc(houston, 100, counties=county, silent=T) %>%    
    ...
    match_numbers(houston$cd) %>%
    mutate(pct_hisp = group_frac(houston, hvap, vap))
