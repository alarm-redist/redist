...
```{r}
houston_plans = redist_smc(houston_merged, 1500, counties=county)
print(houston_plans)
```
...
houston = read_rds("../vra-f20/data/clean/tx.rds") %>%
    redist_map(n_distr=max(cd), pop_tol=0.05, existing_col=cd) %>%
    filter(cd %in% c(2, 7, 9, 18, 22, 29)) %>% # filter to Houston area
    mutate(cd_cores = make_cores(within=1)) %>%
    merge_by(cd_cores)

houston_plans = redist_smc(houston_area, 1500, county=county) %>%    
